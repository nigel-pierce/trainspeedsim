...
def load_maxspeeds():
    import csv
    raw_maxspeeds = [maxspeed for maxspeed in map (MaxSpeed._make, csv.reader(open("sprinter_maxspeeds4.csv", "r"), delimiter='	', quoting=csv.QUOTE_NONNUMERIC))]
    ...
    for x in range(0, len(raw_maxspeeds)-1):
        ...
        if (x+1 == len(raw_maxspeeds)-1):
            seg = TrackSeg(mp1, mp2, mp2-mp1, maxguy2.speed * 5280/3600)
